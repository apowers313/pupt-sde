...
describe("design-architecture.prompt", () => {
  beforeEach(() => {
  ...
  it("renders successfully", async () => {
    const result = await renderPrompt("design-architecture.prompt", {
      requirements: "Build a real-time chat application with message persistence",
    });
  ...
  it("interpolates input values", async () => {
    const result = await renderPrompt("design-architecture.prompt", {
      requirements: "UNIQUE_REQUIREMENTS_MARKER_456",
    });
    expect(result.text).toContain("UNIQUE_REQUIREMENTS_MARKER_456");
  });
